...
{
	if(!str) die("Database_find: str must have a value", conn);
	...
	for(i = 0; i < db->max_rows; i++)
	{
		struct Address *row = &db->rows[i];
		if(row->set)
		{
			if(strstr(row->name, str) || strstr(row->email, str))
			{
				Address_print(row);
			}
		}
...
void Database_clear(struct Connection *conn)
{
	int i = 0;
	struct Database *db = conn->db;
	...
	{
		struct Address *row = &db->rows[i];
		if(row->set) Database_delete(conn, i);
	}
}
...
int main(int argc, char *argv[])
{
	...
	switch(action)
	{
		...
		case 'f':
			if(argc != 4) die("Need a criteria to search", conn);
